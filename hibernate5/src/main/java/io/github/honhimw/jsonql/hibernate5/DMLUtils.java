...
public class DMLUtils {
    ...
    private EntityTypeImpl<Object> initEntity(Table table, MetadataBuildingContext metadataBuildingContext, SessionFactoryImplementor sessionFactory) {
        ...
        EntityTypeImpl<Object> entityType = new EntityTypeImpl<>(null, null, rootClass, sessionFactory);
        IdentifiableTypeDescriptor.InFlightAccess<Object> inFlightAccess = entityType.getInFlightAccess();
        Iterator<org.hibernate.mapping.ForeignKey> foreignKeyIterator = table.getForeignKeyIterator();
        List<org.hibernate.mapping.ForeignKey> foreignKeys = IteratorUtils.toList(foreignKeyIterator);
        ...
        List<org.hibernate.mapping.Column> foreignColumns = foreignKeys.stream()
            .map(Constraint::getColumns)
            .flatMap(Collection::stream)
            .toList();
        Iterator<org.hibernate.mapping.Column> columnIterator = table.getColumnIterator();
        List<org.hibernate.mapping.Column> rootColumns = IteratorUtils.toList(columnIterator);
        ...
        for (org.hibernate.mapping.Column rootColumn : rootColumns) {
...
//            }
            ...
            SingularAttributeImpl<Object, ?> attribute = new SingularAttributeImpl<>(
                ...
                new BasicTypeImpl<>(javaType, Type.PersistenceType.BASIC),
                null,
                isId,
                false,
...
//            return firstRow + pageSize;
        ...
        private DelegateCriteriaQuery<Tuple> tupleCriteriaQuery() {
            DelegateCriteriaQuery<Tuple> query = new DelegateCriteriaQuery<>((CriteriaQueryImpl<Tuple>) cb.createTupleQuery());
            DelegateRoot<Object> root = (DelegateRoot<Object>) query.from(entityType);
        ...
        private DelegateCriteriaQuery<Long> countCriteriaQuery() {
            DelegateCriteriaQuery<Long> query = new DelegateCriteriaQuery<>((CriteriaQueryImpl<Long>) cb.createQuery(Long.class));
            DelegateRoot<Object> root = (DelegateRoot<Object>) query.from(entityType);
        ...
        private Tuple2<String, Map<String, Object>> jpaQL(DelegateCriteriaQuery<?> criteriaQuery) {
        ...
        private Tuple2<String, List<Object>> jdbcQL(DelegateCriteriaQuery<?> criteriaQuery, boolean limit) {
            ...
            if (limit) {
                if (Objects.nonNull(rowSelection)) {
                    jdbcQL = limitHandler.processSql(jdbcQL, rowSelection);
                    if (log.isDebugEnabled()) {
                        log.debug("with limit SQL: {}", jdbcQL);
                    }
                }
                try {
                    col += limitHandler.bindLimitParametersAtStartOfQuery(rowSelection, fakePreparedStatement, col);
                } catch (Exception e) {
            ...
            if (limit) {
                try {
                    col += limitHandler.bindLimitParametersAtEndOfQuery(rowSelection, fakePreparedStatement, col);
                } catch (Exception e) {
                    throw new IllegalStateException(e);
                }
            }
