...
public class MysqlColumnModifier extends ColumnModifier {
    ...
    public String type(Type type, @Nullable Integer length) {
        ...
        String alterTableString = alterTableString();
        return "%s modify column %s %s".formatted(alterTableString, column.getQuotedName(), dialect.getTypeName(type.sqlTypes(metadata)[0], column.getLength(), column.getPrecision(), column.getScale()));
    }
}
