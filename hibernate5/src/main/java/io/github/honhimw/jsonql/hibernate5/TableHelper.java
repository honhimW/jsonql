...
public class TableHelper {
    ...
    public TableHelper addColumn(Consumer<ColumnBuilder> cbc) {
        ...
        if (Objects.nonNull(col.type())) {
            SimpleValue value = new SimpleValue(metadataBuildingContext, _table);
            if (col.generated) {
    ...
    public TableHelper addIndex(String... columnNames) {
        ...
        _index.setName("idx");
        List<Column> list = IteratorUtils.toList(_table.getColumnIterator()).stream().filter(column -> ArrayUtils.contains(columnNames, column.getName())).toList();
    ...
    public TableHelper addForeign(Table joinTable, Map<String, String> mappings) {
        List<Column> referencedTableColumns = IteratorUtils.toList(joinTable.getColumnIterator());
        ...
        mappings.forEach((joinColumnName, referencedColumnName) -> {
            Column joinColumn = IteratorUtils.toList(_table.getColumnIterator()).stream().filter(column -> StringUtils.equals(joinColumnName, column.getName())).findFirst().orElseThrow();
