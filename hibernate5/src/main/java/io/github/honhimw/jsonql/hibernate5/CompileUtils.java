...
//        fieldName = camelToSnake(fieldName);
    ...
    public static void typeValidate(String name, Type type, JsonNode value, boolean nullable) {
        if (nullable && (value.isNull() || value.isMissingNode())) {
            return;
        }
        if (type instanceof StringNVarcharType || type instanceof TextType) {
            ...
            valid(value.isNumber(), "column[%s] require a FLOAT type, but [%s] is provided.".formatted(name, value.getNodeType().name()));
        }
    }
